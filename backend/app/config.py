...
class Settings(BaseSettings):
    ...
    COMFY_OUTPUT_DIR: str = Field("C:/ComfyUI/output", env="COMFY_OUTPUT_DIR")
    ...
    PAYMENT_MIN_AMOUNT: int = Field(1, env="PAYMENT_MIN_AMOUNT")
    PAYMENT_MAX_AMOUNT: int = Field(10000, env="PAYMENT_MAX_AMOUNT")
    PAYMENT_RETURN_URL: Optional[str] = Field(None, env="PAYMENT_RETURN_URL")
    PAYMENT_POINTS_PER_RUBLE: int = Field(1, env="PAYMENT_POINTS_PER_RUBLE")

    # Weekly bonus scheduler
    WEEKLY_BONUS_ENABLED: bool = Field(True, env="WEEKLY_BONUS_ENABLED")
    WEEKLY_BONUS_AMOUNT: int = Field(10, env="WEEKLY_BONUS_AMOUNT")
...
def ensure_directories():
    for path in (settings.COMFY_INPUT_DIR, settings.COMFY_OUTPUT_DIR):
        dir_path = Path(path)
